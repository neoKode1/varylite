...
<section class="relative min-h-[100dvh] flex items-center justify-center overflow-hidden">
	...
	<div class="absolute inset-0">
		<div class="absolute inset-0 bg-gradient-to-b from-surface-950 via-surface-950 to-surface-900"></div>
		<div class="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full bg-accent-600/10 blur-[120px] animate-pulse-glow"></div>
		<div class="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] rounded-full bg-blue-600/8 blur-[100px] animate-pulse-glow" style="animation-delay: 1.5s;"></div>
		<!-- Grid pattern -->
	...
	<div class="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 text-center">
		<!-- Badge -->
		<div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass text-xs font-medium text-surface-300 mb-8 animate-fade-in">
			<span class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
			AI-Powered Scene Generation
		</div>

		<!-- Headline -->
		<h1 class="text-5xl sm:text-7xl lg:text-8xl font-black tracking-tight leading-[0.9] animate-slide-up">
			<span class="block text-white">IMAGINE</span>
			<span class="block gradient-text mt-1">YOUR WORLD</span>
		</h1>

		<p class="mt-6 text-lg sm:text-xl text-surface-400 max-w-2xl mx-auto leading-relaxed animate-slide-up text-balance" style="animation-delay: 0.1s;">
			The free AI app made for creation by creators. Generate stunning images and videos
			with 70+ models — no sign-up required.
		</p>
		...
		<div class="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 animate-slide-up" style="animation-delay: 0.2s;">
			<a
				href="/generate"
				class="group relative inline-flex items-center gap-3 px-8 py-4 bg-white text-surface-950 font-bold text-lg rounded-xl hover:bg-surface-100 transition-all duration-300 hover:scale-105 shadow-2xl shadow-white/10"
			>
				<span>Start Creating</span>
				<svg class="w-5 h-5 group-hover:translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
			</a>
			<a
				href="/gallery"
				class="inline-flex items-center gap-2 px-8 py-4 text-surface-300 hover:text-white font-medium text-lg rounded-xl border border-white/10 hover:border-white/20 hover:bg-white/5 transition-all duration-300"
			>
				View Gallery
			</a>
		</div>
		...
		<div class="mt-16 animate-slide-up" style="animation-delay: 0.35s;">
			<p class="text-xs text-surface-500 uppercase tracking-widest mb-4">Powered by</p>
			<div class="flex flex-wrap justify-center gap-3">
				{#each models as model, i}
					<button
						class="px-4 py-2 rounded-lg text-sm font-medium transition-all duration-500
							{i === currentModel
								? 'glass glow-border text-white scale-105'
								: 'text-surface-500 hover:text-surface-300 bg-white/[0.02] hover:bg-white/[0.04]'}"
	...
	<div class="absolute bottom-8 left-1/2 -translate-x-1/2 animate-float">
		<svg class="w-5 h-5 text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
...
<!-- ═══ Stats bar ═══ -->
<section class="border-y border-white/5 bg-surface-950/80">
	<div class="mx-auto max-w-6xl px-4 sm:px-6 py-12">
		<div class="grid grid-cols-2 md:grid-cols-4 gap-8">
			{#each stats as stat, i}
				<div class="text-center animate-slide-up" style="animation-delay: {i * 0.08}s;">
					<p class="text-3xl sm:text-4xl font-black text-white">{stat.value}</p>
...
<section class="py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-16">
			<h2 class="text-3xl sm:text-4xl font-bold text-white">Everything You Need to Create</h2>
			<p class="mt-4 text-lg text-surface-400 max-w-2xl mx-auto text-balance">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each features as feature, i}
				<div
					class="group relative p-6 rounded-2xl glass hover:bg-white/[0.06] transition-all duration-300 hover:-translate-y-1 animate-slide-up"
					style="animation-delay: {i * 0.06}s;"
				>
					<!-- Icon -->
					<div class="w-12 h-12 rounded-xl bg-accent-600/10 border border-accent-500/20 flex items-center justify-center text-accent-400 mb-4 group-hover:scale-110 transition-transform">
						{@html feature.icon}
					</div>
...
<section class="py-24 sm:py-32 border-t border-white/5">
	<div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-16">
			...
			<p class="mt-4 text-lg text-surface-400">Three steps to your first creation</p>
		</div>

		<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
			{#each [
				{ step: '01', title: 'Upload or Describe', desc: 'Drop in reference images or write a detailed text prompt describing your scene.' },
				{ step: '02', title: 'Choose Your Model', desc: 'Pick from 70+ AI models optimized for images, videos, avatars, and more.' },
				{ step: '03', title: 'Generate & Download', desc: 'Hit generate and download your creation. Everything is saved in your browser.' },
			] as item, i}
				<div class="text-center animate-slide-up" style="animation-delay: {i * 0.1}s;">
					<div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-surface-900 border border-white/10 mb-6">
						<span class="text-2xl font-black gradient-text">{item.step}</span>
					</div>
					<h3 class="text-lg font-semibold text-white mb-2">{item.title}</h3>
					<p class="text-sm text-surface-400 leading-relaxed">{item.desc}</p>
				</div>
			{/each}
		</div>
	</div>
...
<!-- ═══ Model showcase ═══ -->
<section class="py-24 sm:py-32 border-t border-white/5 bg-surface-900/30">
	<div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-16">
			<h2 class="text-3xl sm:text-4xl font-bold text-white">Powered by the Best Models</h2>
			<p class="mt-4 text-lg text-surface-400">Access premium AI from top providers, all in one place.</p>
		</div>
		...
		<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
			{#each [
				...
				<div
					class="group flex flex-col items-center gap-2 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all duration-300 animate-slide-up"
					style="animation-delay: {i * 0.04}s;"
				>
					<span class="text-sm font-semibold text-surface-300 group-hover:text-white transition-colors">{provider.name}</span>
					<span class="text-[10px] text-surface-600 uppercase tracking-wider">{provider.category}</span>
				</div>
			{/each}
		</div>
	</div>
...
<section class="py-24 sm:py-32 border-t border-white/5">
	<div class="mx-auto max-w-4xl px-4 sm:px-6 text-center">
		...
		<p class="mt-6 text-lg text-surface-400 max-w-xl mx-auto text-balance">
		...
		<div class="mt-10">
			<a
				href="/generate"
				class="group inline-flex items-center gap-3 px-10 py-5 bg-white text-surface-950 font-bold text-xl rounded-2xl hover:bg-surface-100 transition-all duration-300 hover:scale-105 shadow-2xl shadow-white/10"
			>
				<span>Start vARYLite</span>
				<svg class="w-6 h-6 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
			</a>
		</div>
	</div>
</section>
